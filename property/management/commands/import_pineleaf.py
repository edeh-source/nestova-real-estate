...
REQUIRES — Property.price field must allow null before running:
─────────────────────────────────────────────────────────────────
    # In your Property model:
    price = models.DecimalField(
        max_digits=15, decimal_places=2,
        null=True, blank=True,
        help_text='Leave blank if price not yet confirmed from source',
    )
...
WHY NULL?
─────────────────────────────────────────────────────────────────
    The Pineleaf Estates site embeds prices inside flyer images
    (e.g. "6,000,000" is painted onto a JPEG). Standard DOM/JSON-LD
    scraping cannot read image-embedded text. Rather than storing a
    fake placeholder like ₦1 or ₦0, we store NULL so the property
    can be published as "Price on Request" and updated later.
...
─────────────────────────────────────────────────────────────────
    ...
    # Load from a local JSON file downloaded from Apify
    ...
    # Skip image downloads (much faster; text data only)
    python manage.py import_pineleaf --dataset-id <ID> --skip-images

    # Re-import and overwrite price/description if a better value exists
    python manage.py import_pineleaf --dataset-id <ID> --update-existing

SETUP
─────────────────────────────────────────────────────────────────
    pip install requests Pillow
    Set APIFY_API_TOKEN in your .env or Django settings,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # ── 2. Bootstrap shared FK objects (once per run) ─────────────────────
        if not self.dry_run:
        ...
        for idx, item in enumerate(items, 1):
            ...
            try:
                ...
                if result == 'imported':
                    imported += 1
                    # _import_one stamps item['_had_price'] as a side-effect
        ...
        if no_price:
            self.stdout.write(self.style.WARNING(
                ...
                f'with price=NULL (price is embedded in flyer image — '
                f'update manually or re-run after the scraper is improved).'
            ))
    ...
    def _bootstrap_state(self) -> State:
    ...
    def _get_or_create_city(self, name: str) -> City:
        name = (name or DEFAULT_CITY_NAME).strip().title()[:100]
    ...
    def _parse_price(raw) -> Optional[Decimal]:
        """
        Convert '₦45,000,000', '6000000', or '0' → Decimal or None.

        Returns None — NOT Decimal('0') — when the value is missing, zero,
        or unparseable. This lets Property.price be stored as NULL so the
        listing appears as "Price on Request" rather than ₦0 or a fake ₦1.

        CHANGED from original:
            Before → returned Decimal('0') for empty/zero inputs
            After  → returns None, matching the nullable model field
        """
        if not raw:
            return None
        ...
        try:
            ...
            # Zero is "no price known", not a real price of ₦0
    ...
    def _map_type(scraped_type: str, title: str = '') -> str:
        ...
        key = (scraped_type or '').strip().lower().replace('-', '_')
    ...
    @staticmethod
    def _parse_sqft(value, bedrooms: int = 0) -> int:
        """Return a reasonable square footage value."""
        try:
            v = int(value or 0)
    ...
    def _import_one(self, item: dict) -> str:
        ...
        Side-effect: sets item['_had_price'] = True/False so the caller
        can count how many properties were saved without a price.
        """
        title       = (item.get('title') or '').strip()
        ...
        # ── Price ─────────────────────────────────────────────────────────────
        # The updated pageFunction.js returns two price keys:
        #   rawPrice → original string e.g. "₦6,000,000 Actual Prize" or ""
        #   price    → JS cleanPrice() result e.g. "6000000" or "0"
        #
        # Prefer rawPrice (richer content for _parse_price to strip).
        # Fall back to price (pre-cleaned) if rawPrice is absent.
        # If both are empty / "0", price resolves to None → stored as NULL.
        raw_price: str             = item.get('rawPrice') or item.get('price') or ''
        price: Optional[Decimal]   = self._parse_price(raw_price)

        # Stamp result onto item so handle() can count no-price imports
        item['_had_price'] = price is not None
        ...
        # ── DRY RUN ──────────────────────────────────────────────────────────
        if self.dry_run:
            price_display = f'₦{float(price):,.0f}' if price is not None else 'Price TBD'
            self.stdout.write(
                f'       ↳ [{status_code}] {type_code} | '
                f'{price_display} | {bedrooms}bd/{bathrooms}ba | {city_name}'
            )
            return 'dry_run'
        ...
        with transaction.atomic():
            ...
            if existing and self.update_existing:
                ...
                # Only overwrite price when we have a real scraped value.
                # Never replace a known price with NULL.
                ...
                prop = Property(
                    ...
                    price=price,            # ← None when scraper couldn't read it;
                                            #   requires price=DecimalField(null=True, blank=True)
            ...
            # ── Amenities ─────────────────────────────────────────────────────
            if action == 'imported':
                self._save_amenities(prop, item.get('amenities') or [])

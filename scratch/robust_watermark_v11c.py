"""
robust_watermark_v11c.py

Using a STATIC MASK for inpainting.
Since PropertyPro applies the exact same watermark overlay to every image,
we can extract a perfect mask from one "easy" image and reuse it for all images.
This completely eliminates false positives (like the TV or painting) being smeared!
"""
...
# We will generate a base mask once, dynamically, from an "easy" image
# or by aggregating a few images.
def generate_static_mask(w, h):
    # For a perfect static mask, we can just create a mask that covers the exact
    # text "PropertyPro.ng" and the house icon. 
    # Since doing that mathematically from scratch is hard, we can just use the
    # difference method on a few images, take the MINIMUM across them (so random
    # bright background objects disappear, as they don't appear in the same spot
    # on every image), and we are left with ONLY the watermark!
    ...
    for url in TEST_URLS:
        ...
        # Resize to standard size for aggregation
    ...
    # Take the median/intersection of all masks
    # The watermark is the ONLY thing that is bright/desaturated in the EXACT same
    # pixel locations across all 5 completely different images.
    masks = np.array(masks)
    
    # We want pixels where at least 3 out of 5 images agree it's a watermark
    consensus = np.sum(masks == 255, axis=0)
    static_band_mask = (consensus >= 3).astype(np.uint8) * 255
    
    # Clean up and dilate
    kernel = np.ones((5, 5), np.uint8)
    static_band_mask = cv2.dilate(static_band_mask, kernel, iterations=3)
    
    # Return as a function that can resize this canonical mask to any image size
...
def main():
    print("Generating static consensus mask...")
    get_mask_fn = generate_static_mask(800, 600)
    
    # Save the canonical mask just to see it
    cv2.imwrite(os.path.join(OUT_DIR, "v11c_canonical_mask.jpg"), get_mask_fn(800, 600))
    ...
    for i, url in enumerate(TEST_URLS, 1):
        ...
        try:
            ...
            # Inpaint using the perfect static mask
            ...
            out_path = os.path.join(OUT_DIR, f"v11c_{i}_comparison.jpg")
